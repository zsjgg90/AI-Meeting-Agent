...
import { KnowledgeMeetingItem, listKnowledgeMeetings } from '../api';
import { KnowledgeMeetingCard } from '../components/KnowledgeCard';
...
export function KnowledgeMeetingListScreen({ onBack, onOpenMeeting }: Props) {
  const [items, setItems] = useState<KnowledgeMeetingItem[]>([]);
  ...
  const load = useCallback(async (mode: 'initial' | 'refresh' | 'more' = 'initial') => {
    ...
    try {
      ...
      setError(null);
      const response = await listKnowledgeMeetings({
        query,
      ...
      setError(nextError instanceof Error ? nextError.message : '会议记录加载失败');
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>会议记录</Text>
      </View>
      <View style={styles.searchRow}>
        <View style={styles.searchBox}>
          <LucideIcon name="search" color="#8b95a7" size={18} strokeWidth={2.2} />
          <TextInput value={query} onChangeText={setQuery} onSubmitEditing={() => load('refresh')} placeholder="搜索会议标题..." placeholderTextColor="#9ca3af" style={styles.searchInput} />
        </View>
      ...
      <Text style={styles.count}>共 {total} 场会议</Text>
      ...
      {!loading && !error ? (
        <FlatList
          ...
          renderItem={({ item }) => <KnowledgeMeetingCard item={item} onPress={onOpenMeeting} />}
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => load('refresh')} tintColor="#6657ff" />}
          onEndReached={() => load('more')}
          onEndReachedThreshold={0.25}
          ListEmptyComponent={<KnowledgeEmptyState title="暂无会议记录" text="完成会议分析并同步知识后会在这里展示。" />}
