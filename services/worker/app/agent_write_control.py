...
from app.agent_contract import AgentActionProposal, EvidenceRef, RiskLevel
from app.agent_state_tracker import TrackedObjectType
...
class InMemoryAuthoritativeStateProvider:
    def __init__(self, snapshots: list[AuthoritativeStateSnapshot] | None = None) -> None:
        self._snapshots: dict[tuple[str, str], AuthoritativeStateSnapshot] = {}
        self.reads: list[dict[str, str]] = []
        for snapshot in snapshots or []:
            self.add(snapshot)

    def add(self, snapshot: AuthoritativeStateSnapshot) -> None:
        self._snapshots[(snapshot.object_type, snapshot.object_id)] = snapshot

    def get_state(self, *, object_type: TrackedObjectType, object_id: str) -> AuthoritativeStateSnapshot | None:
        self.reads.append({"object_type": object_type, "object_id": object_id})
        return self._snapshots.get((object_type, object_id))
...
class ControlledWritePlanner:
    ...
    def plan(
        ...
        *,
        ...
        snapshot = self._read_authoritative_state(
...
def non_whitelisted_fields(proposal: AgentActionProposal) -> list[str]:
    ...
    fields = set(normalize_command_changes(proposal.proposed_changes))
...
def _has_permission(
    ...
    *,
    ...
    accepted = {
        "*",
        "agent_write",
        f"agent_write:{target_type}",
        f"{operation}:{target_type}",
        f"{operation}:*",
    }
...
__all__ = [
    "AGENT_WRITE_CONTROL_SCHEMA_VERSION",
