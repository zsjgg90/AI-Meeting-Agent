from app.meeting_analyst_service import analyze_meeting as analyze_meeting_with_rag
...
def save_summary(db: Session, meeting: Meeting, payload: dict[str, Any]) -> MeetingSummary:
    ...
    summary = MeetingSummary(
        ...
        rag_chunk_ids=metadata.get("rag_chunk_ids") if isinstance(metadata.get("rag_chunk_ids"), list) else [],
...
def segment_to_analyst_dict(segment: TranscriptSegment) -> dict[str, Any]:
    return {
...
def confidence_score(items: list[dict[str, Any]]) -> float | None:
    values: list[float] = []
    for item in items:
...
def normalize_agenda_items(items: Any) -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(items, start=1):
        if isinstance(item, str):
            text = item.strip()
            if text:
                normalized.append({"item": text, "status": "已讨论", "summary": text, "order": index})
        elif isinstance(item, dict):
            text = str(item.get("item") or item.get("title") or item.get("summary") or "").strip()
            if text:
                next_item = dict(item)
                next_item.setdefault("item", text)
                next_item.setdefault("status", "已讨论")
                next_item.setdefault("summary", text)
                next_item.setdefault("order", index)
                normalized.append(next_item)
    return normalized


def normalize_dict_items(items: Any) -> list[dict[str, Any]]:
    if not isinstance(items, list):
        return []
    return [dict(item) for item in items if isinstance(item, dict)]


def qwen_rag_result_to_payload(result: dict[str, Any]) -> dict[str, Any]:
    settings = get_settings()
    meeting_agenda = normalize_agenda_items(result.get("meeting_agenda", []))
    key_conclusions = normalize_dict_items(result.get("key_conclusions", []))
    action_items = normalize_dict_items(result.get("action_items", []))
    unresolved_issues = normalize_dict_items(result.get("unresolved_issues", []))
    risks_and_focus = normalize_dict_items(result.get("risks_and_focus", []))
    meeting_summary = str(result.get("meeting_summary") or result.get("summary") or "").strip()

    confidence_items = [*key_conclusions, *action_items, *unresolved_issues, *risks_and_focus]
    ...
    return {
        ...
        "next_steps": [
            {
                ...
                "time": item.get("deadline") or "",
                "note": item.get("source_text", ""),
            }
        ...
        "confidence_score": confidence_score(confidence_items),
    }


def summarize_meeting(db: Session, meeting_id: str) -> MeetingSummary:
    log_event("summary.started", meeting_id=meeting_id)
    ...
    try:
        ...
        db.commit()
        transcript_rows = [segment_to_analyst_dict(segment) for segment in segments]
        with log_stage("transcript_build", meeting_id=meeting_id, segment_count=len(segments)):
        ...
        with log_stage(
            ...
            prompt_version=payload.get("metadata", {}).get("prompt_version"),
            rag_chunk_ids=payload.get("metadata", {}).get("rag_chunk_ids", []),
        ):
            summary = save_summary(db, meeting, payload)
        log_event(
            "summary.completed",
            meeting_id=meeting_id,
            summary_id=summary.id,
